use studfinder::{Config, StudFinder, ScanQuality, ProcessorType};
...
async fn test_color_confidence_decreases_with_impurity() {
    ...
    // Scan both images
    let pure_piece = finder.scan_image(pure_path.path().to_path_buf()).await.unwrap();
    let impure_piece = finder.scan_image(impure_path.path().to_path_buf()).await.unwrap();
